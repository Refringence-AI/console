...
import { Cable, GitBranch, Rocket, Loader2, FileWarning, Folder, FolderOpen, Play, GitPullRequest } from 'lucide-react';
import { toast } from 'sonner';
import { useQueryClient } from '@tanstack/react-query';
import { PanelHeader } from '../_shell/PanelHeader';
import { usePipelineDetect, usePipelineRuns, usePrLink } from '../../lib/queries/pipeline';
 ...
 * always capitalized.
 */
// AN-7: tint the PR badge by state so a merged/closed PR reads differently from
// an open one at a glance. Falls back to the neutral border for unknown states.
function prStateClasses(state?: string): string {
    switch (state) {
        case 'OPEN': return 'border-emerald-500/40 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400';
        case 'MERGED': return 'border-violet-500/40 bg-violet-500/10 text-violet-600 dark:text-violet-400';
        case 'CLOSED': return 'border-rose-500/40 bg-rose-500/10 text-rose-600 dark:text-rose-400';
...
function PipelineSeasoned() {
    ...
    const prLink = usePrLink(projectRoot);
    ...
    return (
        <div className="flex h-full min-h-0 flex-col overflow-y-auto" data-testid="pipeline-panel">
            ...
            >
                ...
                {prLink.data?.found && prLink.data.url && (
                    <button
                        type="button"
                        data-testid="pipeline-pr-link"
                        onClick={() => void bridge.openExternal(prLink.data!.url!)}
                        title={prLink.data.title ?? `Pull request #${prLink.data.number}`}
                        className={`hidden items-center gap-1 rounded-md border px-2 py-1 text-label sm:inline-flex ${prStateClasses(prLink.data.state)}`}
                    >
                        <GitPullRequest className="h-3 w-3" />
                        <span className="tabular-nums">#{prLink.data.number}</span>
                        <span className="font-medium">{prLink.data.state}</span>
                    </button>
                )}
                <Button
                    variant="secondary"
