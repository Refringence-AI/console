...
import { takePendingPrompt } from '../../lib/prompts/pending';
...
import { HandoffHistoryButton } from './HandoffHistory';
...
export function PromptLibraryPanel() {
    ...
    const filtered = useMemo(() => {
        ...
        return list.filter((p) => {
            ...
            if (q) {
                const hay = `${p.title} ${p.body} ${p.whatWhen ?? ''} ${p.tags.join(' ')} ${p.category}`.toLowerCase();
                if (!hay.includes(q)) return false;
            }
            return true;
        });
    }, [list, query, category, tag, favOnly, source]);
    ...
    useEffect(() => {
        ...
        // The command palette can hand off a specific prompt id to open.
        const pending = takePendingPrompt();
        selectPrompt((pending && list.find((p) => p.id === pending)) || list[0]);
    ...
    // Built-ins are read-only; cloning makes an editable copy in the user's
    // library and selects it (preserving the variable hints).
    async function clone(p: PromptEntry) {
        try {
            ...
            toast.error(e instanceof Error ? e.message : 'Could not clone the prompt');
        }
    }
    ...
    return (
        <div className="flex h-full flex-col" data-testid="prompts-panel">
            <PanelHeader icon={BookMarked} title="Prompts" subtitle="Reusable prompt templates and agent skills">
                <HandoffHistoryButton root={root} />
                <SkillsButton root={root} />
                <Button type="button" variant="outline" size="sm" onClick={() => setImportOpen(true)} className="gap-1.5" data-testid="prompt-import-open">
                    ...
                    <div className="flex flex-col gap-2 border-b border-border p-3">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5">
                            ...
                            <FilterChip
                                label="Mine"
                                active={source === 'user'}
                                onClick={() => setSource((s) => (s === 'user' ? 'all' : 'user'))}
                            />
                            <FilterChip
                                label="Curated"
                                active={source === 'builtin'}
                                onClick={() => setSource((s) => (s === 'builtin' ? 'all' : 'builtin'))}
                            />
                            {categories.map((c) => (
                                <FilterChip
                                    key={c}
                        ...
                        {tags.length > 0 && (
                            <div className="flex flex-wrap items-center gap-1.5">
                                {/* The curated catalogue brings ~35 tags; collapse them by
                                    default so the list column stays calm. Categories +
                                    search already cover most filtering. */}
                                <FilterChip
                                    label={showTags ? 'Hide tags' : `Tags (${tags.length})`}
                                    active={showTags}
                                    onClick={() => setShowTags((v) => !v)}
                                />
                                {(showTags || tag) && tags.map((t) => (
                                    <FilterChip
                                        key={t}
                                        label={`#${t}`}
                                        active={tag === t}
                                ...
                                <li key={p.id}>
                                    ...
                                    >
                                        ...
                                        <span className="flex items-center gap-1.5">
                                            ...
                                            {p.source === 'builtin' && (
                                                <Badge variant="outline" className="rounded-sm text-label text-accent">
                                                    Curated
                                                </Badge>
                                            )}
