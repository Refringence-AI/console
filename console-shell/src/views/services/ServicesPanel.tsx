    ...
    return (
        <div className="flex h-full min-h-0 flex-col overflow-y-auto" data-testid="services-panel">
            ...
            <div className="flex flex-col gap-8 p-6">
                ...
                <EnvConnectCard onConnected={invalidateConnections} />
                <ConnectorsSection />
                {CATEGORIES.map((cat) => {
...
function EnvConnectCard({ onConnected }: { onConnected: () => void }) {
    const qc = useQueryClient();
    ...
    async function connect(id: string, name: string) {
        setBusy(id);
        try {
            const r = await bridge.env.connect(root, id);
            if (r.ok) {
                toast.success(`Linked ${name} from .env${r.detail ? ` (${r.detail})` : ''}`);
                onConnected();
                void qc.invalidateQueries({ queryKey: ['ai', 'keyStatus'] });
                void scan.refetch();
            } else {
                toast.error(r.error ?? `Could not link ${name}`);
            }
        } catch (err) {
    ...
    if (scan.isLoading) {
        return (
            <section className="flex flex-col gap-3" data-testid="services-env-scanning">
                <SectionLabel>From your .env</SectionLabel>
                <UICard className="flex items-center gap-2.5 p-4 shadow-none">
                    <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin text-muted-foreground" />
                    <p className="text-small text-muted-foreground">Scanning your <code className="font-mono text-label">.env</code> for connectable services…</p>
                </UICard>
            </section>
        );
    }
    ...
    if (scan.isError) {
        return (
            <section className="flex flex-col gap-3" data-testid="services-env-error">
                <SectionLabel>From your .env</SectionLabel>
                <UICard className="flex items-center justify-between gap-2.5 p-4 shadow-none">
                    <p className="text-small text-muted-foreground">Could not read your <code className="font-mono text-label">.env</code> this time.</p>
                    <Button size="sm" variant="outline" onClick={() => void scan.refetch()}>Retry</Button>
                </UICard>
            </section>
        );
    }
    ...
    return (
        <section className="flex flex-col gap-3" data-testid="services-env-connect">
            <SectionLabel>From your .env</SectionLabel>
            <UICard className="flex flex-col gap-3 p-4 shadow-none">
                <div className="flex items-start gap-2.5 rounded-lg border border-border bg-secondary/30 px-3 py-2.5">
                    ...
                    <p className="text-small leading-relaxed text-muted-foreground">
                        <span className="font-medium text-foreground">Your keys stay private.</span> Console found these from your <code className="font-mono text-label">.env</code> key NAMES, never the values. Tap Link and the value is read transiently to test and store the connection, then dropped - never shown back, never sent to AI.
                    </p>
                </div>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    {items.map((s) => (
                        <div key={s.id} className="flex items-center gap-2 rounded-lg border border-border bg-background px-3 py-2" data-testid={`env-connect-${s.id}`}>
                            <div className="min-w-0 flex-1">
                                <p className="text-small font-medium text-foreground">{s.name}</p>
                                <p className="truncate font-mono text-label text-muted-foreground">{s.keyName}</p>
                            </div>
                            <Button size="sm" variant="outline" disabled={busy === s.id} onClick={() => connect(s.id, s.name)}>
                                {busy === s.id ? 'Linking…' : 'Link'}
                            </Button>
                        </div>
                    ))}
                </div>
