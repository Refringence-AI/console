import { Activity, FolderOpen, ExternalLink, ChevronDown, ChevronRight, FileText, FlaskConical, PlayCircle, TestTube, Zap, GitBranch, CheckCircle2, XCircle, AlertCircle, Database, Loader2 } from 'lucide-react';
...
import type { RunEntry, SentryIssue, RunnerStartOpts, DbSaturation } from '../../lib/bridge';
import { Card, Badge, Button, SectionLabel, Stat, StatLabel, StatValue } from '@/components/ui';
import { cn } from '@/lib/utils';

/**
 * Observability panel - surfaces .refringence-console/runs/ artifacts +
...
function ObservabilitySeasoned() {
    ...
    return (
        <div className="flex h-full min-h-0 flex-col overflow-y-auto bg-background" data-testid="observability-panel">
            ...
            <SectionDivider label="Database saturation" />
            <div className="px-6">
                <DbSaturationCard projectRoot={projectRoot} />
            </div>
...
// A live Postgres saturation probe (idle-in-tx, longest query, pool usage). The
// connection string is held only in local state and sent once; the main process
// runs SELECTs only, closes the connection immediately, and never stores it.
function DbSaturationCard({ projectRoot }: { projectRoot: string }) {
    const [conn, setConn] = useState('');
    const [busy, setBusy] = useState(false);
    const [result, setResult] = useState<DbSaturation | null>(null);
    async function check() {
        if (!conn.trim() || !projectRoot) return;
        setBusy(true);
        try { setResult(await bridge.db.saturation(projectRoot, conn.trim())); }
        finally { setBusy(false); }
    }
    const tiles: { label: string; value: string; tone?: 'rose' | 'neutral' }[] = result && result.ok ? [
        { label: 'Pool used', value: result.pctUsed != null ? `${Math.round(result.pctUsed)}%` : '-', tone: (result.pctUsed ?? 0) >= 80 ? 'rose' : 'neutral' },
        { label: 'Connections', value: result.usedConns != null && result.maxConns != null ? `${result.usedConns}/${result.maxConns}` : '-' },
        { label: 'Idle in tx', value: result.idleInTx != null ? String(result.idleInTx) : '-', tone: (result.idleInTx ?? 0) > 0 ? 'rose' : 'neutral' },
        { label: 'Blocked', value: result.blocked != null ? String(result.blocked) : '-', tone: (result.blocked ?? 0) > 0 ? 'rose' : 'neutral' },
        { label: 'Longest query', value: result.longestQuerySecs != null ? `${result.longestQuerySecs.toFixed(1)}s` : '-' },
    ] : [];
    return (
        <Card className="gap-3 p-4" data-testid="db-saturation">
            <p className="text-small leading-relaxed text-muted-foreground">
                Probe a Postgres for connection-pool saturation. Use a read-only connection string; it is sent once, never stored.
            </p>
            <form className="flex items-center gap-2" onSubmit={(e) => { e.preventDefault(); void check(); }}>
                <input
                    type="password"
                    value={conn}
                    onChange={(e) => setConn(e.target.value)}
                    placeholder="postgres://readonly:…@host:5432/db"
                    data-testid="db-saturation-conn"
                    className="h-8 min-w-0 flex-1 rounded-md border border-border bg-background px-2.5 font-mono text-small text-foreground outline-none focus:border-accent"
                />
                <Button type="submit" size="sm" disabled={busy || !conn.trim() || !projectRoot} data-testid="db-saturation-check">
                    {busy ? <Loader2 className="size-3.5 animate-spin" /> : <Database className="size-3.5" />} Check
                </Button>
            </form>
            {result && !result.ok && (
                <p className="text-small text-warning-text" data-testid="db-saturation-error">{result.error ?? 'Could not read database saturation.'}</p>
            )}
            {result && result.ok && (
                <div className="flex flex-col gap-3" data-testid="db-saturation-metrics">
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
                        {tiles.map((t) => (
                            <div key={t.label} className="rounded-lg bg-secondary/40 px-3 py-2">
                                <p className="text-label uppercase tracking-wide text-muted-foreground">{t.label}</p>
                                <p className={cn('text-section tabular-nums', t.tone === 'rose' ? 'text-danger-text' : 'text-foreground')}>{t.value}</p>
                            </div>
                        ))}
                    </div>
                    {result.warnings && result.warnings.length > 0 && (
                        <div className="flex flex-col gap-1" data-testid="db-saturation-warnings">
                            {result.warnings.map((wmsg) => (
                                <div key={wmsg} className="flex items-start gap-1.5 text-label">
                                    <AlertCircle className="mt-0.5 size-3 shrink-0 text-warning-text" />
                                    <span className="text-muted-foreground">{wmsg}</span>
                                </div>
                            ))}
                        </div>
                    )}
...
function RunDetailFiles({
    ...
    return (
        <div className="flex flex-col gap-1" data-testid={`obs-run-detail-${runId.slice(0, 24)}`}>
            {files.map((f) => {
                const fullPath = `.refringence-console/runs/${runId}/${f.relPath}`;
                return (
                    <div
                        key={f.relPath}
    ...
    if (loading) {
        return (
            <Card className="gap-0 p-4 text-body text-muted-foreground shadow-none">
                Scanning .refringence-console/runs.
            </Card>
        );
    }
...
function ArtifactLog({ runs }: { runs: RunEntry[] }) {
    ...
    return (
        <div className="flex flex-col gap-2" data-testid="obs-artifact-log">
            {runs.map((r) => {
                const folderPath = `.refringence-console/runs/${r.runId}`;
