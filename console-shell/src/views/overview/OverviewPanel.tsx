        ...
        <div className="flex h-full flex-col overflow-y-auto" data-testid="overview-panel">
            ...
            <Section title="Vitals" className="mt-7">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <VitalCell to="/observability">
                        <Stat>
                            <StatLabel icon={Coins}>Provider spend</StatLabel>
                            {metrics.data ? (
                                <>
                                    <StatValue>
                                        {metrics.data.cost_today_usd === null ? (
                                    ...
                                    <StatHint>
                                        {metrics.data.cost_today_usd === null ? 'Connect a provider to track spend' : 'across connected providers'}
                                    </StatHint>
                                </>
                            ) : <CellSkeleton />}
                        </Stat>
    ...
    if (alerts.length === 0) {
        return (
            <div className="flex items-center gap-2 rounded-lg border border-success/25 bg-success/[0.07] px-3.5 py-2 text-body text-success-text">
                ...
                <span className="font-medium">No blockers.</span>
                <span className="text-success-text/70">Nothing is failing; anything still pending is listed below.</span>
            </div>
        );
    }
    return (
