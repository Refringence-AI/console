...
function buildCatalogue(actions: { id: string; label: string; icon: LucideIcon; description: string; run: () => void }[]): PaletteItem[] {
    ...
    return [...panels, ...actionItems, ...docs];
}
...
export default function CommandPalette({
    ...
    const catalogue = useMemo(() => buildCatalogue(actions), [actions]);

    const orderedItems = useMemo(() => {
        const q = debouncedQuery.trim().toLowerCase();
        if (!q) return alphabetical(catalogue);
