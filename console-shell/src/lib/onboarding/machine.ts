...
export type OnbStage =
    | 'persona' | 'tour' | 'signin' | 'ai' | 'connect' | 'study' | 'report' | 'intent' | 'services' | 'done';
...
const GUIDED_ORDER: OnbStage[] = ['persona', 'tour', 'signin', 'ai', 'connect', 'study', 'report', 'intent', 'services', 'done'];
const OPERATOR_ORDER: OnbStage[] = ['persona', 'ai', 'connect', 'study', 'report', 'intent', 'services', 'done'];
...
function milestoneId(stage: OnbStage): string | null {
    switch (stage) {
        ...
        case 'ai': return 'ai';
        case 'connect': case 'study': case 'report': return 'project';
