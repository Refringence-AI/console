...
is_exempt() {
  ...
  # Vendored React Bits background (copied verbatim from reactbits.dev) is
  # upstream third-party code; its comments may legitimately contain em-dashes.
  case "$f" in
    */components/Grainient/*) return 0 ;;
  esac
  case "$(basename "$f")" in
