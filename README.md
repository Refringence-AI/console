<div align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/status-v0.9.0--rc-orange.svg" alt="Release candidate" />
  </p>
</div>
...
- **Prompts**: a library of reusable prompt templates with typed `{{variables}}`.
  Fill the form, preview the result, then copy it, send it to the assistant, or
  hand it to your AI coding tool.
- **Tool config**: the rules, permissions, and MCP servers your AI coding tools
  run under (Claude Code, Cursor, Copilot, Windsurf, Codex, Gemini), parsed from
  the repo's own config files and shown side by side, no AI needed.
- **Design system**: the project's palette, type scale, spacing, design tokens,
  and component libraries, extracted from Tailwind config, `@theme` tokens,
  `components.json`, and dependencies, and saved as a profile you can reuse.
- **Workboard**: issues and PRs as a Kanban board plus a table, synced from the
  ...
  relabel the issue in place. A PR-first repo with no issues gets an honest empty
  state that points at its pull requests.
...
With an AI provider connected, the built-in assistant works as a project
operator, not just a chat box. It reads the open project's real files (release
checks, dependency licenses, migration drift, dead config) and, behind a
diff-and-approve gate, can write a file, generate a deploy or CI config, write an
SBOM, or run a command to verify a change. A read-only Plan mode lets it
investigate and propose a plan first, changing nothing until you approve. A
command it runs is always confirmed on its own, so approving a file write never
silently authorizes a shell command.
