    ...
    try {
        ...
        for await (const part of result.fullStream) {
            ...
            if (type === 'text-delta') {
                ...
                // The SDK reports cumulative usage on the finish part; shape
                // varies a little by version, so probe the common fields.
                const u = (part.totalUsage ?? part.usage) as { inputTokens?: number; outputTokens?: number; totalTokens?: number } | undefined;
                if (u) params.onUsage?.(u);
            } else if (type === 'error') {
                const e = part.error;
