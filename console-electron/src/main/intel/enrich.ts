...
import { pkgForRelPath } from '../ipc/architecture-graph';
...
// Fold the deterministic dependency graph into the AI's semantic diagram (SC-5).
// The model is the only source of edges, so a sparse response renders as rows of
// unconnected cards (a list). Here we (a) classify each AI edge as a real import
// (detected) vs inference, (b) backfill the real cross-system imports the model
// missed so the graph regains its connective tissue, and (c) attach per-system
// coupling so nodes read as grounded structure. All additive; absent depGraph is
// a no-op.
export function groundDiagram(diagram: SystemDiagram, profile: ProjectProfile): SystemDiagram {
    const dep = profile.depGraph;
    if (!dep || dep.nodes.length === 0) return diagram;

    // Map every system's validated paths to their dependency-graph package ids.
    const pkgToSystem = new Map<string, string>();
    const sysPkgs = new Map<string, Set<string>>();
    for (const n of diagram.nodes) {
        const pkgs = new Set<string>();
        for (const p of n.paths) pkgs.add(pkgForRelPath(p));
        sysPkgs.set(n.id, pkgs);
        for (const pk of pkgs) if (!pkgToSystem.has(pk)) pkgToSystem.set(pk, n.id);
    }

    // Real cross-system import weights, directed.
    const cross = new Map<string, number>(); // `${src} ${dst}` -> weight
    for (const e of dep.edges) {
        const sa = pkgToSystem.get(e.source);
        const sb = pkgToSystem.get(e.target);
        if (!sa || !sb || sa === sb) continue;
        const k = `${sa} ${sb}`;
        cross.set(k, (cross.get(k) ?? 0) + (e.weight || 1));
    }
    const between = (a: string, b: string) => (cross.get(`${a} ${b}`) ?? 0) + (cross.get(`${b} ${a}`) ?? 0);

    // Node grounding: size from the dep nodes, system-level fan from cross edges,
    // orphan = no real import crosses this system's boundary either way.
    const depById = new Map(dep.nodes.map((n) => [n.id, n]));
    const groundedNodes: SystemNode[] = diagram.nodes.map((n) => {
        let loc = 0, fileCount = 0;
        for (const pk of sysPkgs.get(n.id) ?? []) { const d = depById.get(pk); if (d) { loc += d.loc; fileCount += d.fileCount; } }
        const fanOut = diagram.nodes.filter((o) => o.id !== n.id && (cross.get(`${n.id} ${o.id}`) ?? 0) > 0).length;
        const fanIn = diagram.nodes.filter((o) => o.id !== n.id && (cross.get(`${o.id} ${n.id}`) ?? 0) > 0).length;
        return { ...n, loc: loc || undefined, fileCount: fileCount || undefined, fanIn, fanOut, isOrphan: fanIn === 0 && fanOut === 0 };
    });

    const pairKey = (a: string, b: string) => (a < b ? `${a} ${b}` : `${b} ${a}`);
    const aiPairs = new Set<string>();
    const classified: SystemEdge[] = diagram.edges.map((e) => {
        aiPairs.add(pairKey(e.source, e.target));
        const w = between(e.source, e.target);
        return w > 0 ? { ...e, via: 'detected' as const, weight: w } : { ...e, via: 'inferred' as const };
    });

    // Backfill real cross-system imports the model never drew, top-weighted first.
    const backfill: SystemEdge[] = [];
    const seen = new Set(aiPairs);
    for (const [k, w] of [...cross.entries()].sort((a, b) => b[1] - a[1])) {
        const [src, dst] = k.split(' ');
        const pk = pairKey(src, dst);
        if (seen.has(pk)) continue;
        seen.add(pk);
        backfill.push({ source: src, target: dst, label: 'imports', via: 'detected', weight: w });
        if (backfill.length >= 14) break;
    }

    return { nodes: groundedNodes, edges: [...classified, ...backfill] };
}
...
export async function enrichProfile(profile: ProjectProfile, modelId: string | undefined, nowIso: string): Promise<EnrichResult> {
    ...
    // Fold the real dependency graph into the AI diagram: classify + backfill
    // edges from actual imports, attach per-system coupling.
    const groundedDiagram = diagram ? groundDiagram(diagram, profile) : null;

    const intel: ProjectIntel = {
        narrative, tagline, systemDiagram: groundedDiagram, suggestions,
