...
export interface SystemNode {
    ...
    paths: string[];
    summary: string;
    // Deterministic grounding folded in from the dependency graph (SC-5). All
    // optional so a diagram persisted before this lands stays valid.
    loc?: number;
    fileCount?: number;
    fanIn?: number;       // packages outside this system that import into it
    fanOut?: number;      // packages outside this system this one imports
    isOrphan?: boolean;   // no real import crosses this system's boundary
}
...
export interface SystemEdge {
    ...
    /** Summed import weight when detected. */
    weight?: number;
}
