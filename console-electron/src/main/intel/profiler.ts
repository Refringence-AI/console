...
import { readEnvConsent } from './consent';
...
export function deriveCommands(root: string): RunCommand[] {
    const out: RunCommand[] = [];
    const seen = new Set<string>();
    const push = (group: RunCommand['group'], name: string, cmd: string, pkg: string, key: string) => {
        if (out.length >= 120 || seen.has(key)) return;
        seen.add(key);
        out.push({ group, name, cmd: cmd.slice(0, 200), pkg });
    };
    for (const pj of findPackageJsons(root, 60)) {
        const pkg = readJson(pj);
        ...
        for (const [name, cmd] of Object.entries(scripts as Record<string, unknown>)) {
            ...
            push(groupForScript(name), name, cmd, rel, `${rel}:${name}`);
        }
    }
    const mk = readText(path.join(root, 'Makefile'), 8000);
    for (const m of mk.matchAll(/^([a-zA-Z][a-zA-Z0-9_-]*):/gm)) {
        push(groupForScript(m[1]), m[1], `make ${m[1]}`, '.', `make:${m[1]}`);
    }
    // justfile recipes (just). A recipe is `name:` or `name args:`; the negative
    // lookahead skips just's `name := value` assignments, and `#` lines are
    // never matched by the leading-identifier anchor.
    const just = readText(path.join(root, 'justfile'), 8000) || readText(path.join(root, 'Justfile'), 8000) || readText(path.join(root, '.justfile'), 8000);
    for (const m of just.matchAll(/^([a-zA-Z_][a-zA-Z0-9_-]*)[^:\n]*:(?!=)/gm)) {
        push(groupForScript(m[1]), m[1], `just ${m[1]}`, '.', `just:${m[1]}`);
    }
    // Taskfile (go-task): task names are the keys under the top-level `tasks:`.
    const taskFile = readText(path.join(root, 'Taskfile.yml'), 12000) || readText(path.join(root, 'Taskfile.yaml'), 12000);
    if (taskFile) {
        let inTasks = false;
        for (const line of taskFile.split('\n')) {
            ...
            const tm = line.match(/^ {2}(['"]?)([a-zA-Z0-9_:-]+)\1:\s*$/) ?? line.match(/^ {2}(['"]?)([a-zA-Z0-9_:-]+)\1:\s/);
            if (tm) push(groupForScript(tm[2]), tm[2], `task ${tm[2]}`, '.', `task:${tm[2]}`);
        }
    }
    return out;
}
...
export async function buildProfile(
    ...
    // .env key NAMES are read only if the user has not explicitly declined in
    // onboarding (null = never asked -> read, for repos profiled before the gate).
    const envNames = readEnvConsent(root) !== false ? collectEnvNames(root) : [];
    ...
    const workspace = detectWorkspace(root, scan);
...
function detectWorkspace(root: string, scan: TreeScan): WorkspaceInfo {
    ...
    // Real sub-packages = package.json files below the root (the scan already
    // skips node_modules). A loose root script or a plain src/ dir is not a
    // package, so this is the honest monorepo signal - not the top-level node
    // count, which over-reports a single app with a few root files as a monorepo.
    let subPackages = 0;
    for (const rel of scan.relFiles) if (/\/package\.json$/.test(rel)) subPackages += 1;
    ...
    else if (subPackages >= 2) tool = 'npm --prefix';
    // A monorepo if a workspace tool is declared OR there are >=2 real packages.
    const isMonorepo = tool !== null && tool !== 'npm --prefix' ? true : subPackages >= 2;
