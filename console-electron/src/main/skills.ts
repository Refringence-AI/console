...
// Custom skills the user writes live per-project at
// <root>/.refringence-console/skills.json (parallel to prompts.json) so they
// travel with the repo. They install to .claude/.codex exactly like the
// built-ins; the only difference is they are editable.
...
interface CustomSkillStore { version: number; skills: SkillDef[]; }

const CUSTOM_STORE = ['.refringence-console', 'skills.json'];

function customPath(root: string): string {
    return path.join(path.resolve(root), ...CUSTOM_STORE);
}
...
function isCustomSkill(v: unknown): v is SkillDef {
    if (!v || typeof v !== 'object') return false;
    const o = v as Record<string, unknown>;
    return typeof o.id === 'string' && typeof o.name === 'string'
        && typeof o.description === 'string' && typeof o.body === 'string';
}

export function readCustomSkills(root: string): SkillDef[] {
    try {
        const parsed = JSON.parse(fs.readFileSync(customPath(root), 'utf8')) as Partial<CustomSkillStore>;
        if (!Array.isArray(parsed.skills)) return [];
        return parsed.skills.filter(isCustomSkill).map((s) => ({
...
function writeCustomSkills(root: string, skills: SkillDef[]): boolean {
    const file = customPath(root);
    const tmp = `${file}.${process.pid}.tmp`;
    try {
        fs.mkdirSync(path.dirname(file), { recursive: true });
        fs.writeFileSync(tmp, JSON.stringify({ version: 1, skills }, null, 2), 'utf8');
...
function sanitizeSkill(input: SkillInput): { name: string; description: string; body: string; tags: string[] } | null {
    if (!input || typeof input !== 'object') return null;
    if (typeof input.name !== 'string' || input.name.trim().length === 0) return null;
    if (typeof input.body !== 'string' || input.body.trim().length === 0) return null;
    return {
...
// Returns the full custom skills (with body) so the editor can load them.
export function listCustomSkills(root: string): SkillDef[] {
    if (!root || typeof root !== 'string') return [];
    return readCustomSkills(root);
}

export function createSkill(root: string, input: SkillInput): SkillMeta | null {
    if (!root || typeof root !== 'string') return null;
    const clean = sanitizeSkill(input);
    if (!clean) return null;
    const skills = readCustomSkills(root);
    const taken = new Set([...SKILLS.map((s) => s.id), ...skills.map((s) => s.id)]);
    let id = slugify(clean.name);
    while (taken.has(id)) id = `${slugify(clean.name)}-${Math.random().toString(36).slice(2, 6)}`;
    const def: SkillDef = { id, ...clean };
    skills.unshift(def);
    if (!writeCustomSkills(root, skills)) return null;
    const { body: _body, ...meta } = def;
    return meta;
}

export function updateSkill(root: string, id: string, input: SkillInput): SkillMeta | null {
...
export function deleteSkill(root: string, id: string): boolean {
    if (!root || typeof id !== 'string') return false;
    const skills = readCustomSkills(root);
    const next = skills.filter((s) => s.id !== id);
    if (next.length === skills.length) return false;
    return writeCustomSkills(root, next);
}

// A skill by id, looking in the built-in catalogue first, then the user's.
function findSkill(root: string, id: string): SkillDef | null {
    return SKILLS.find((s) => s.id === id) ?? readCustomSkills(root).find((s) => s.id === id) ?? null;
}
...
// Which skills (built-in or user-authored) are already installed for a tool.
export function installedSkills(root: string, tool: SkillTool): string[] {
    const out: string[] = [];
    const ids = [...SKILLS.map((s) => s.id), ...readCustomSkills(root).map((s) => s.id)];
    for (const id of ids) {
        try {
            if (fs.existsSync(path.join(skillDir(root, tool, id), 'SKILL.md'))) out.push(id);
        } catch { /* ignore */ }
    }
    return out;
}
