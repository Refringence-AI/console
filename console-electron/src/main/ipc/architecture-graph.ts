...
// A deterministic architecture finding the UI surfaces as a side list + node
// badge. confidence is medium where the signal can have a legitimate cause
// (a public entry point, a heuristic tier), so Console never tells a maintainer
// to delete live code.
...
// Per-package coupling, in Robert Martin's terms. instability = fanOut /
// (fanIn + fanOut): 0 = nothing depends on it being stable, 1 = pure consumer.
    ...
    while (stack.length > 0) {
        ...
        for (const ent of entries) {
            ...
            if (ent.isDirectory()) {
                ...
                // A loose file directly in the repo root (no '/') is not its own
                // package; bucket all such root config/script files under one
                // (root) node instead of emitting one node per file.
                const pkg = rel.includes('/') ? pkgForRelPath(rel) : '(root)';
...
interface ResolveContext {
    root: string;
    fileToPkg: Map<string, WalkedFile>;
    /** Valid package ids (the walked packages), to gate directory-fallback resolution. */
    pkgs: Set<string>;
    /** A workspace package.json "name" -> its package id, for by-name imports. */
...
function pkgForResolvedPath(base: string, ctx: ResolveContext): string | null {
    const exts = ['', '.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.mts', '.cts'];
    for (const e of exts) {
        const hit = ctx.fileToPkg.get(base + e);
        if (hit) return hit.pkg;
    }
    const indices = [
        'index.ts', 'index.tsx', 'index.js', 'index.jsx', 'index.mjs', 'index.cjs', 'index.mts', 'index.cts',
        'src/index.ts', 'src/index.tsx', 'src/index.js', 'src/index.jsx',
    ];
    for (const idx of indices) {
        const hit = ctx.fileToPkg.get(path.join(base, idx));
        if (hit) return hit.pkg;
    }
    // A directory import into a walked package that has no top-level index
    // (e.g. packages/x/src/foo.ts, or a package whose entry is deeper): map the
    // directory to its package id so the cross-package edge is still created.
    // Gated on the package being real, and self-imports drop out as no-ops.
    const rel = path.relative(ctx.root, base).replace(/\\/g, '/');
    if (rel && !rel.startsWith('..')) {
        const pkg = pkgForRelPath(rel);
        if (ctx.pkgs.has(pkg)) return pkg;
    }
    return null;
}

// Map each workspace package.json "name" to its package id, so a by-name import
// (`@scope/ui`) resolves to the internal package even with no alias or declared
// dep. The root manifest is skipped (it is the workspace, not a graph node).
function collectPackageNames(walk: WalkResult, root: string): Map<string, string> {
    const out = new Map<string, string>();
    for (const file of walk.packageJsons) {
        const rel = path.relative(root, path.dirname(file)).replace(/\\/g, '/');
        if (rel === '') continue;
        const pkg = pkgForRelPath(rel);
        if (!walk.pkgStats.has(pkg)) continue;
        try {
            const j = JSON.parse(fs.readFileSync(file, 'utf8')) as { name?: unknown };
            if (typeof j.name === 'string' && j.name) out.set(j.name, pkg);
...
function resolveJsImport(fromAbs: string, spec: string, ctx: ResolveContext): string | null {
    if (spec.startsWith('.')) {
        ...
        return pkgForResolvedPath(base, ctx);
    }
    for (const alias of ctx.tsAliases) {
        ...
        for (const target of alias.targets) {
            ...
            const pkg = pkgForResolvedPath(base, ctx);
            if (pkg) return pkg;
        }
    }
    // A bare specifier that names a workspace package by its package.json "name"
    // (with or without a /subpath) is an internal cross-package edge, even when
    // no tsconfig alias maps it and the importer never declared it as a dep.
    const byName = ctx.pkgNameToId.get(bareModuleName(spec));
...
export function computeGraph(
    ...
    const ctx: ResolveContext = {
        ...
        pkgs: new Set(walk.pkgStats.keys()),
        pkgNameToId: collectPackageNames(walk, root),
    ...
    return {
        ...
        edges,
        cycles,
        insights: deriveInsights(nodes, edges, cycles, inbound, outbound),
        truncated: walk.truncated,
        fileCount: walk.totalFiles,
    };
}

// Outer (entry) -> inner (core) layering rank. Healthy edges flow outer->inner;
// an inner layer importing an outer one is a layering smell. test imports
// anything; external is the innermost leaf.
const TIER_RANK: Record<ArchTier, number> = {
    test: 5, shell: 4, presentation: 3, domain: 2, infra: 1, data: 1, external: 0,
};

// Deterministic architecture findings + coupling metrics over the package graph.
// Pure function of (nodes, edges, cycles, fan counts); no I/O.
function deriveInsights(
    ...
    const internal = nodes.filter((n) => n.id !== '(root)' && !n.external);
    const tierById = new Map(internal.map((n) => [n.id, n.tier]));
    const labelById = new Map(nodes.map((n) => [n.id, n.label]));
    const lbl = (id: string): string => labelById.get(id) ?? id;

    const metrics: NodeMetric[] = internal.map((n) => {
        const fanIn = inbound.get(n.id) ?? 0;
        const fanOut = outbound.get(n.id) ?? 0;
        const tot = fanIn + fanOut;
        return { id: n.id, fanIn, fanOut, instability: tot === 0 ? 0 : Number((fanOut / tot).toFixed(2)) };
    });
    ...
    for (const cyc of cycles) {
        ...
        findings.push({
            kind: 'cycle', severity: 'high', confidence: 'high',
            title: `Dependency cycle (${cyc.length} packages)`,
            detail: `${cyc.map(lbl).join(' -> ')} -> ${lbl(cyc[0])}. A cycle means these packages cannot be built, tested, or understood on their own.`,
            nodes: cyc,
        });
    }

    // Orphans only mean something in a real multi-package repo; in a single app
    // the lone src node has no importer because it IS the entry, not an orphan.
    if (internal.length >= 3) {
        for (const n of internal) {
            if ((inbound.get(n.id) ?? 0) > 0) continue;
            if (n.tier !== 'domain' && n.tier !== 'data') continue;
            findings.push({
                kind: 'orphan', severity: 'medium', confidence: 'medium',
                title: `Nothing imports ${n.label}`,
                detail: `No other package in this repo imports ${n.label}. It may be unused, or it may be a public entry point. Confirm before removing it.`,
                nodes: [n.id],
            });
        }
    }
    ...
    for (const e of edges) {
        const st = tierById.get(e.source); const tt = tierById.get(e.target);
        if (!st || !tt || st === 'test') continue;
        if (TIER_RANK[st] < TIER_RANK[tt]) {
            findings.push({
                kind: 'layer-violation', severity: 'medium', confidence: 'medium',
                title: `${lbl(e.source)} (${st}) depends on ${lbl(e.target)} (${tt})`,
                detail: `Dependencies usually flow toward more stable, lower-level layers. A ${st} package importing a ${tt} one is often an inverted dependency worth a second look.`,
                nodes: [e.source, e.target],
            });
        }
    }

    const hubThreshold = Math.max(4, Math.ceil(internal.length * 0.4));
    for (const m of metrics) {
        if (m.fanIn < hubThreshold) continue;
        findings.push({
            kind: 'hub', severity: 'info', confidence: 'high',
            title: `${lbl(m.id)} is central (${m.fanIn} dependents)`,
            detail: `${m.fanIn} packages depend on ${lbl(m.id)}. Keep it stable and well tested; a change here reaches many places.`,
            nodes: [m.id],
        });
    }

    const sev: Record<ArchFinding['severity'], number> = { high: 0, medium: 1, info: 2 };
...
// 4: root-level loose files collapse into one (root) bucket instead of one node
// per file. 5: by-name + directory edge resolution (recovers cross-package
// edges). 6: insights (findings + metrics) attached to the graph. Stale caches
// from an older version must be discarded.
