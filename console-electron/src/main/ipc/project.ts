...
export function registerProjectHandlers(): void {
    ...
    // The renderer reports the active project whenever it changes, so the next
    // launch reopens it. Persisted in userData (survives reopen + app updates).
    ipcMain.handle('console:project.remember', (_evt, root: string): void => {
        if (typeof root === 'string' && root.length > 0) rememberLastProject(root);
    });
