...
import { getCrossVendorSpend } from '../connectors/spend';
...
export interface MetricsSummary {
    ...
    // null = AI spend is not tracked yet (no provider usage wired). The UI shows
    // an explicit "Not tracked" state, never a misleading "$0.00".
...
export function registerMetricsHandlers(): void {
    ipcMain.handle('console:metrics.summary', async (_e, root: string): Promise<MetricsSummary> => {
        ...
        return {
            ...
            // Real spend summed across connected provider connectors (cached,
            // non-blocking - the first read may be null until the background
            // refresh lands). null when nothing reports a cost, which the UI
            // renders as "Not tracked" rather than a misleading "$0.00".
            cost_today_usd: getCrossVendorSpend(),
