...
export interface CutResult { ok: boolean; tag?: string; error?: string }

// Cut a release = create an annotated git tag for the package.json version,
// gated on no failing (red) or blocked gate. Local only; pushing the tag + a
// GitHub release is a separate step that needs the remote.
export async function cutRelease(root: string): Promise<CutResult> {
    ...
    if (!version) return { ok: false, error: 'No version in package.json. Set one before cutting a release.' };
    const tag = version.startsWith('v') ? version : `v${version}`;

    const s = summarise(await computeGates(root));
    if (s.red > 0 || s.blocked > 0) {
        return { ok: false, error: `Release is not ready: ${s.red} failing, ${s.blocked} blocked. Resolve those gates first.` };
    }
    const existing = await gitOut(root, ['tag', '-l', tag]);
    if (existing === null) return { ok: false, error: 'Not a git checkout, so there is nothing to tag.' };
    if (existing.trim() === tag) return { ok: false, error: `Tag ${tag} already exists. Bump the version or roll it back first.` };

    const made = await gitOut(root, ['tag', '-a', tag, '-m', `Release ${tag}`]);
    if (made === null) return { ok: false, error: 'Could not create the tag.' };
    return { ok: true, tag };
}

// Roll back a just-cut release by deleting its local tag. Does not touch a remote.
export async function rollbackRelease(root: string, tag: string): Promise<{ ok: boolean; error?: string }> {
    if (typeof root !== 'string' || root.length === 0) return { ok: false, error: 'No project is open.' };
    if (typeof tag !== 'string' || !/^v?[\w.\-]+$/.test(tag)) return { ok: false, error: 'Invalid tag name.' };
    const res = await gitOut(root, ['tag', '-d', tag]);
    return res === null ? { ok: false, error: `Could not delete tag ${tag} (does it exist?).` } : { ok: true };
}
...
export function registerReleaseHandlers(): void {
    ...
    ipcMain.handle('console:release.cut', async (_e, root: string): Promise<CutResult> => {
        try { return await cutRelease(root); } catch (err) { return { ok: false, error: err instanceof Error ? err.message : String(err) }; }
    });

    ipcMain.handle('console:release.rollback', async (_e, root: string, tag: string): Promise<{ ok: boolean; error?: string }> => {
        try { return await rollbackRelease(root, tag); } catch (err) { return { ok: false, error: err instanceof Error ? err.message : String(err) }; }
    });
}
