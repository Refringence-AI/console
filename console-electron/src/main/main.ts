...
import { readLastProject } from './lastProject';
...
function projectPathFromArgv(argv: string[]): string | undefined {
    ...
    try {
        ...
        // Never adopt Console's own app/dev tree as a project. A dev run
        // (`electron .`), a launcher, or a test harness can pass the app
        // directory (e.g. console-electron) as argv; opening that as the user's
        // project is the long-standing "it auto-opened console-electron" bug.
        const appDir = path.resolve(app.getAppPath());
        if (resolved === appDir || resolved.startsWith(appDir + path.sep) || appDir.startsWith(resolved + path.sep)) {
            return undefined;
        }
...
if (!gotInstanceLock) {
    ...
    app.whenReady().then(() => {
        ...
        // First window honours a project path passed on the command line, else
        // reopens the last project the user had (a returning user lands back on
        // their work); a fresh install has none and shows onboarding.
        mainWindow = createConsoleWindow(projectPathFromArgv(process.argv) ?? readLastProject());

        app.on('activate', () => {
            if (BrowserWindow.getAllWindows().length === 0) {
                mainWindow = createConsoleWindow(readLastProject());
            }
        });
    });
}
